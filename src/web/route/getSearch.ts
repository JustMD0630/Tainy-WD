...
export async function getSearch(
  ...
  // Use the validated source (validSource) instead of raw source
  const result = await client.rainlink
    .search(query, { requester: user, engine: validSource })
    .catch(() => ({
  ...
  if (result.tracks.length == 0 && result.playlistName == 'dreamvast@error@noNode') {
    // Return 200 with empty result instead of 404 to avoid console errors
    res.send({
      type: RainlinkSearchResultType.SEARCH,
      playlistName: null,
      tracks: []
  ...
  res.send({
    ...
    tracks: result.tracks.map((track) => {
      ...
      return {
        ...
        author: track.author,
        sourceName: track.source, // Add sourceName
        encoded: track.encoded,   // Add encoded track string

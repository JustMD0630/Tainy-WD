...
export default class implements Command {
  ...
  public async execute(client: Manager, handler: CommandHandler) {
    ...
    // If it's a user, also update the User schema to reflect the ban
    if (type === 'user') {
      const user = await client.db.user.get(id) || {
        id,
        banned: false,
        muted: false,
        warns: 0,
      ...
      user.banned = true
      user.banLevel = 3 // Owner Blacklist = Level 3
      user.banReason = 'Blacklisted by Bot Owner'
      user.banExpires = 0 // Permanent
      
      await client.db.user.set(id, user)
    }
  ...
  protected async removeData(
    ...
    // If it's a user, also remove the ban from User schema
    if (type === 'user') {
      const user = await client.db.user.get(id)
      if (user && user.banLevel === 3) { // Only remove if it was a level 3 ban (blacklist)
        user.banned = false
        user.banLevel = 0

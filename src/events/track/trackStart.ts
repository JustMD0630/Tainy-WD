...
export default class {
  async execute(client: Manager, player: RainlinkPlayer, track: RainlinkTrack) {
    ...
    const requesterText = (() => {
      const r: any = song.requester
      if (!r) return client.i18n.get(language, 'event.player', 'request_unknown') ?? 'Unknown'
      if (typeof r === 'string') return r
      if (typeof r === 'object') {
        return r.tag ?? r.username ?? r.globalName ?? r.displayName ?? r.id ?? 'Unknown'
      }
      return 'Unknown'
    })()

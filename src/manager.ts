...
export class Manager extends Client {
  ...
  public start() {
    this.logger.info('ClientManager', `Booting tainy...`)
    ...
    if (this.config.utilities.WEB_SERVER.enable) {
      // Only start WebServer on the first cluster (Cluster 0) or if not sharded
      if (!this.cluster.data || this.cluster.id === 0) {
        new WebServer(this)
      } else {
        this.logger.info('ClientManager', `Skipping WebServer for Cluster ${this.cluster.id}`)
      }
